...
<template>
  ...
  <form @submit.prevent="submitForm()">
    ...
    <div class="mb-3">
      ...
      <span v-if="validationErrors.role" class="text-danger"
            style="text-align: center; display: block ruby">{{ validationErrors.role }}</span>
    ...
    <hr>
    <span v-if="validationErrors.address" class="text-danger" style="text-align: center">{{
    ...
    <span v-if="validationErrors.password" class="text-danger" style="text-align: center">{{
...
const validationErrors = ref({});
  ...
  onMounted(() => {
    ...
    // find the staff member
        axios.get(`http://localhost:8080/api/gaaManagementSystem/find-staff/${id}`)
            .then(response => {
...
// PUT request to save user details
    async function submitForm() {
      ...
      try {
        ...
        await axios.put(`http://localhost:8080/api/gaaManagementSystem/update-staff/${this.staff.staff_id}`, this.staff);
